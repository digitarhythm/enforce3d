...
enterframe = ->
  ...
  for obj in _objects
    if (obj.active == true && obj.motionObj? && obj.motionObj.active && typeof(obj.motionObj.behavior) == 'function')
...
addObject = (param, parent = undefined)->
  ...
  type = if (param.type?) then param.type else GLSPHERE
...
setMotionObj = (param)->
  ...
  gravity       = if (param.gravity?)       then param.gravity      else 0.0
  opacity       = if (param.opacity?)       then param.opacity      else 1.0
  type          = if (param.type?)          then param.type         else PRIMITIVE
  motionsprite  = if (param.motionsprite?)  then param.motionsprite else undefined
  motionObj     = if (param.motionObj?)     then param.motionObj    else undefined
  alpha         = if (param.alpha?)         then param.alpha        else 0.0
  beta          = if (param.beta?)          then param.beta         else 0.0
  gamma         = if (param.gamma?)         then param.gamma        else 0.0
  vcanvas       = if (param.vcanvas?)       then param.vcanvas      else undefined
  vtexture      = if (param.vtexture?)      then param.vtexture     else undefined
